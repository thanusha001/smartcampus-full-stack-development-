...
sql = """select * from faculty_assignment where faculty_id='%s'"""%(r[1])
cur.execute(sql)
r=cur.fetchall()
cnt=0
for i in r:
	...
	<tr>
		...
		<td>%s</td>
		<td>%s</td>
		<td><a href='fac_view_subassign.py?fid=%s&id=%s&aid=%s'>View Assignment</a></td>
		</tr>
	"""%(cnt,i[2],i[3],i[4],i[5],i[1],id,i[2]))
print("""</table>
</center>
</div></div><br><br>
